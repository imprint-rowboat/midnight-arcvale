...
const sdk = new CoinbaseWalletSDK({
  appName: "Midnight Arcvale (Built for Base)",
...
async function rpcHealth() {
  const c = client();
  const [chainId, bn, gas] = await Promise.all([c.getChainId(), c.getBlockNumber(), c.getGasPrice()]);
  const ok = chainId === 8453 || chainId === 84532;
  print([
    "RPC Health",
    `Network: ${active.label}`,
    `Reported chainId: ${chainId} (${ok ? "Base-compatible" : "unexpected"})`,
    `Latest block: ${bn}`,
...
function mount() {
  ...
  h1.textContent = "Midnight Arcvale";

  const blurb = document.createElement("div");
  blurb.textContent =
    "Built for Base: wallet connection, chainId validation, and read-only ETH + ERC-20 lookups with Basescan references.";
  ...
  row.append(btn("Connect Wallet", connectWallet), btn("Toggle Network", toggleNetwork), btn("Latest Block", latestBlock), btn("RPC Health", rpcHealth));
  row2.append(
    addrInput,
